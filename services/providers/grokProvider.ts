...
export const analyzeWithGrok = async (
  ...
  const apiKey = process.env.GROK_API_KEY || 
                 (typeof window !== 'undefined' && (window as any).GROK_API_KEY) ||
                 (typeof localStorage !== 'undefined' && localStorage.getItem('GROK_API_KEY'));
  
  if (!apiKey) {
    throw new Error("GROK_API_KEY not found. Please add your Grok API key in Settings to continue.");
  }
...
Return your response as a JSON object with this exact structure:
{
  ...
  try {
    ...
    if (!parsedResponse.clips || !Array.isArray(parsedResponse.clips)) {
      throw new Error('Invalid response format from Grok - missing clips array');
    }

    // Validate and transform clips
    const validatedClips = parsedResponse.clips
      .filter((clip: any) => {
        return clip.title && 
               clip.reason && 
      ...
      .map((clip: any) => {
        // Ensure viral score has all required fields
        const viralScore = {
          overall: clip.viralScore.overall || 0,
          engagement: clip.viralScore.engagement || 0,
          shareability: clip.viralScore.shareability || 0,
          retention: clip.viralScore.retention || 0,
          trend: clip.viralScore.trend || 0
        };
    ...
    if (validatedClips.length === 0) {
      throw new Error('No valid clips found in Grok response');
    }
